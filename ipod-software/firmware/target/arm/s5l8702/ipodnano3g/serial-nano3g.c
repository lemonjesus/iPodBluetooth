...
void tx_blocking_writec(unsigned char c) {
    uartc_port_send_byte(&ser_port, c);
}

int rx_rdy(void) {
    return uartc_port_rx_ready(&ser_port) ? 1 : 0;
}

char rx_getc() {
    return (char) uartc_port_rx_byte(&ser_port);
}

char rx_blocking_getc() {
    return (char) uartc_port_read_byte(&ser_port);
}
...
void serial_setup(void)
{
    ...
    uartc_port_set_bitrate_raw(&ser_port, BRDATA_9600);
