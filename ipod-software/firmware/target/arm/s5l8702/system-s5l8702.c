...
void system_init(void)
{
    ...
    pmu_preinit();
#endif
    // TODO: vic_init();        // XXX: sacarlo de .S
    gpio_init();
    eint_init();
#if 1
vic_init(); // XXX: sacarlo de .S
#endif
    dma_init();
#ifdef HAVE_SERIAL
    uart_init();
...
//     PWRCON(i) = 0;
    ...
    if (sec_epoch)
        CLKCON0 &= ~CLKCON0_SDR_DISABLE_BIT;                // XXX: llamar a esto UNK31_BIT ???
    else
        CLKCON0 |= CLKCON0_SDR_DISABLE_BIT;
...
#else
// XXX: sin esto no funciona la SDRAM !!! !!! !!!
    ...
    // TODO: configurar un ECLK de 12 MHz para todos los targets, 
    // asi la configuracion de los timers sera la misma.
...
// TODO: hay cosas mal, copiar de spireader
static void miu_preinit(bool selfrefreshing)
{
    GPIOUNK384 = 0;
    ...
    if (selfrefreshing)
        MIUCOM = 0x11;      /* TBC: self-refresh -> IDLE */      // XXX: el s5l8702 hace MIU_REG(0) = MIUCO_N_ = 0x11 !!! !!! !!!
    else
    {
        MIUCOM = 0x33;      /* No action CMD */
        MIUCOM = 0x233;     /* Precharge all banks CMD */
...
{
    bool hibernated;        // TODO: hibernated -> resuming, o quizas mejor warmboot
#ifdef IPOD_NANO4G
