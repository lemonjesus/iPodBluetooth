...
static void usage(void)
{
    ...
    "            --mkdfu-raw <filename.bin> <outfile.dfu> [--s5l8720]\n"
    "\n"
    "Commands:\n"
...
int main(int argc, char* argv[])
{
    ...
    while (--argc)
    {
        ...
        if (!memcmp(*argv, "--bl", 4)) {
            ...
            dfu_opt |= DFU_OPT_SINGLE;
        }
        else if (!strcmp(*argv, "--s5l8720") || !strcmp(*argv, "-2")) {
            dfu_opt |= DFU_OPT_S5L8720;
        }
        /* undocummented options: */
        else if (!strcmp(*argv, "--debug")) {
            /* print debug traces while the .dfu file is sent to de iPod */
            ipoddfu_debug(1);
        }
        else if (!strcmp(*argv, "--nonpersistent")) {
            /* do not write the bootloader on flash, just execute it, this
               option should be used with --bl-inst or --mkdfu-inst */
            dfu_opt |= DFU_OPT_NONPERSISTENT;
        }
        else
            usage();
    }
...
//         dfu_type = DFU_INST_SINGLE;
    ...
    if (dfuinfile)
        ...
        dfubuf = mkdfu(dfu_type | (dfu_opt << DFU_OPTIONS_POS),
    ...
    if (dfuoutfile) {
        ...
        /* The DFU file is sent to the requested USB PID device, if --pid is
         * not specified then it is sent to te first DFU device found.
         * TODO: for --bl-{inst|uninst} verify that the bootloader is sent to
