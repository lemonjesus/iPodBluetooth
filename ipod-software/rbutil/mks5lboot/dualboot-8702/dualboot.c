...
/* How it works:
 *
 * - dualboot-installer: installs or updates a RB bootloader into an iPod
 *   device, this installer and the bootloader.ipod file will be included
 *   into a generated dualboot-installer.dfu file (and optionally sent to
 *   the iPod device) using the mks5lboot tool.
 *
 *   When dualboot-installer.dfu is executed by the iPod device:
 *
 *    1) locates an original NOR boot (ONB): first it looks at offset=32KB,
 *       if a NOR boot is found but it is not an ONB then it is supposed it
 *       is a previously installed RB bootloader that should be updated and
 *       the ONB is loaded from offset=32KB+old_BLSIZE).
 ...
 *    3) write the new RB bootloader at NOR offset=32KB, it it fails then
 *       goto 2a).
 *
 * - dualboot-uninstaller: uninstall RB bootloader from NOR, leaving the
 *   iPod at it's previous 'pristine' state.
 *
